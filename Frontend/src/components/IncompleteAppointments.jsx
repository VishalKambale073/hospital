...
const IncompleteAppointments = () => {
    ...
    const fetchAppointments = () => {
        ...
        axios.get(`http://localhost:3000/api/displayincompletedappointments?doctorEmail=${doctorEmail}`)
        .then((res) => {
            console.log("API Full Response:", res); // Log the full response
            console.log("API Response Data:", res.data);

            if (res.data.success) {
                console.log("Incomplete Appointments:", res.data.data);
                setAppData(res.data.data);
            } else {
                console.log("No Incomplete appointments found.");
                setAppData([]);
            }
        })
        .catch((error) => {
